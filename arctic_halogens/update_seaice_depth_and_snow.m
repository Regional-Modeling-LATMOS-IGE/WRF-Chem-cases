...
%
% Louis Marelle, 2020/02/06
%
clear
close all

%-------- Input --------
ASR2_DIR = '/data/marelle/met_data/ASR2';
WPS_DIR = '/data/marelle/WRFChem/WRF_OUTPUT/met_em_test/'


%-------- Parameters --------
SNOWDEPTH_SEAICE = 0.3; % meters, N-ICE campaign, before the melt season begins
USE_ASR2_SEAICE = true;
...
%-------- Initialize --------
% Add my matlab toolbox to the path
...
for idomain = 1:3
  ...
  [moad_cen_lat, truelat1, truelat2, stdlon, imax, jmax, kmax, dx, dy, ref_lat,...
   ref_lon, map_proj, hemi, ref_x, ref_y] = get_WRF_grid([WPS_DIR, '/', met_em_filenames(1, :)]); % Not sure this is needed
  ...
  if (USE_ASR2_SEAICE)
    ...
    % the ASR2 references. So this is not the actual projection, but a close one found
    ...
    % ii=360,jj=360,lat=90; lon=-175; [a, b] = ijll(ii,jj,60,NaN,1,lon,lat,lon,asr2_imax/2+0.5,asr2_jmax/2+0.5,15,2);asr2_xlat(ii,jj)-a,asr2_xlong(ii,jj)-b,
    % ii=360,jj=360,lat=90; lon=-175; [a, b] = ijll(ii,jj,asr2_proj.truelat1,asr2_proj.truelat2, asr2_proj.hemi, asr2_proj.stdlon,asr2_proj.ref_lat, asr2_proj.ref_lon, asr2_proj.ref_x,asr2_proj.ref_y, asr2_proj.dx, asr2_proj.map_proj);asr2_xlat(ii,jj)-a,asr2_xlong(ii,jj)-b,
